...
function is_allowed_topic(string $prompt): bool {
    $allowed_keywords = [
        'gym','olahraga','latihan','workout','fitness','kebugaran','nutrisi','diet','otot','cardio','strength','kesehatan','protein','kalori','fat','lemak','reps','sets','rest','pemanasan','stretching','cedera','recovery'
    ];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$prompt) {
        ...
        if (!is_allowed_topic($prompt)) {
            $response = 'Maaf, saya tidak bisa menjawab karena pertanyaan di luar konteks kesehatan/gym/olahraga.';
        } else {
...
?>
<h2 class="mb-3"><img src="<?= get_base_url(); ?>/img/3.png" alt="AI Coach" class="section-icon"> AI Coach</h2>
