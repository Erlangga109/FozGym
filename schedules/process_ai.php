...
// Determine the effective member_id for whom the schedule is being processed
...
$time_per_session = $_GET['time_per_session'] ?? 45;
...
if (!$has_api_key) {
    flash('error', 'API key belum diset. Tidak bisa memproses jadwal otomatis. Silakan atur OpenAI atau Gemini API key di config.php.');
    header('Location: index.php');
    exit;
}
...
// Fungsi untuk menentukan hari latihan berdasarkan jumlah hari yang tersedia
...
// Proses jadwal jika belum dihasilkan
if (empty($schedule['schedule_data'])) {
    try {
        // Ambil jumlah hari latihan dari database
        $stmt = $pdo->prepare("SELECT COUNT(*) as day_count FROM schedule_days WHERE schedule_id = ?");
        $stmt->execute([$schedule_id]);
        $day_count = $stmt->fetch()['day_count'];
        
        // Hasilkan jadwal dari AI
        $ai_schedule = generate_workout_schedule(
            $schedule['target_muscle_groups'], 
            $schedule['training_goals'], 
            $schedule['difficulty_level'], 
            $day_count, 
            $time_per_session
        );
        
        // Simpan jadwal ke dalam field schedule_data
        $stmt = $pdo->prepare("UPDATE workout_schedules SET schedule_data = ? WHERE id = ?");
        $stmt->execute([json_encode($ai_schedule), $schedule_id]);
        
        // Update hari-hari dengan latihan yang dihasilkan AI
        $stmt = $pdo->prepare("SELECT * FROM schedule_days WHERE schedule_id = ? ORDER BY day_of_week");
        $stmt->execute([$schedule_id]);
        $schedule_days = $stmt->fetchAll();
        
        foreach ($ai_schedule['schedule'] as $day_index => $day_data) {
            if ($day_index < count($schedule_days)) {
                $day = $schedule_days[$day_index];
                
                // Update exercises untuk hari ini
                $stmt_update = $pdo->prepare("UPDATE schedule_days SET exercises = ? WHERE id = ?");
                $stmt_update->execute([json_encode($day_data['exercises']), $day['id']]);
            }
        }
        
        flash('success', 'Jadwal latihan otomatis berhasil dibuat oleh AI!');
        header('Location: view.php?id=' . $schedule_id . '&member_id=' . $effective_member_id);
        exit;
    } catch (Exception $e) {
        flash('error', 'Gagal membuat jadwal otomatis: ' . $e->getMessage());
        header('Location: index.php');
        exit;
    }
} else {
    // Jika jadwal sudah dihasilkan, arahkan ke halaman view
    header('Location: view.php?id=' . $schedule_id . '&member_id=' . $effective_member_id);
    exit;
}